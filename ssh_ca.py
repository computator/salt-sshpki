...
def ext_pillar(
        ...
        pki_root,
        ca_privkey,
        identity_fmt_str='salt_ssh_ca:host:{}',
        validity_period='4w',
    ...
    for key_type, host_key in host_keys.iteritems():
        ...
        if not cert_path or cert_expires < datetime.now() + timedelta(days=reissue_early_days):
            ...
            cert_path = pki.sign_key(identity_fmt_str.format(minion_id), (minion_id,), '-' + str(backdate_days) + 'd:+' + str(validity_period), keystr=host_key, host_key=True)
